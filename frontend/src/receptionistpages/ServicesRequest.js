          ...
          <TableBody>
            {filteredServiceRequestList.map((request) => (
              <TableRow key={request._id}>
                 <TableCell>{request.guest?.name || 'Unknown Guest'}</TableCell>
                 <TableCell>{request.service?.name || 'Unknown Service'}</TableCell>
      ...
      <Box sx={{ display: { xs: 'block', md: 'none' } }}>
        {filteredServiceRequestList.map((request) => (
          <Paper key={request._id} sx={{ p: 2, mb: 2 }}>
            <Typography variant="h6">{request.guest?.name|| 'Unknown Guest'}</Typography>
            <Typography>Service: {request.service?.name|| 'Unknown Service'}</Typography>
